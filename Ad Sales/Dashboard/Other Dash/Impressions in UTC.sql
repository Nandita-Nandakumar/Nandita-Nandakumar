...
(
    SELECT DISTINCT DATE(PARSE_TIMESTAMP('%Y%m%d%H%M%S',event_time)) as event_date, transaction_id, selling_partner_id as reseller_id, COUNT (*) as impressions
    FROM `fubotv-prod.dmp.de_sync_freewheel_logs` t1
    WHERE DATE(PARSE_TIMESTAMP('%Y%m%d%H%M%S',event_time)) >= '2021-03-08'
...
,direct_data AS
(
    SELECT DISTINCT DATE(PARSE_TIMESTAMP('%Y%m%d%H%M%S',event_time)) as event_date, transaction_id, COUNT (*) as impressions
    FROM `fubotv-prod.dmp.de_sync_freewheel_logs` t1
    WHERE DATE(PARSE_TIMESTAMP('%Y%m%d%H%M%S',event_time)) >= '2021-03-08'
    AND event_name = 'defaultImpression' -- sum of default impressions 
...
(
    ...
    FROM `fubotv-prod.FW_Views.FW_Logs_View` 
    ORDER BY 1
)
