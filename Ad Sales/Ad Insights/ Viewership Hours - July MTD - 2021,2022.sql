--- July MTD  - Viewership Hours Request - Jul 1st - 4th year over year data_insights
with users as (
...
, 
LOWER(t4.ad_insertable) AS ad_insertable_network , t1.program_title,t1.user_id, t1.start_time,SUM(duration)/3600 as Hours
...
ELSE 'others'
END AS playback_type, 
'yes' AS ad_insertable_network ,t1.program_title, t1.user_id, t1.start_time, SUM(duration)/3600 as Hours
from `fubotv-prod.data_insights.video_in_progress_via_va_view` t1
inner join users t2 on t1.user_id=t2.user_id and date(t1.start_time)=t2.day
inner join `fubotv-prod.data_insights.tmsid_genre_mapping` t3 on t1.tms_id=t3.tms_id
inner join `fubotv-dev.business_analytics.AdInsertable_Networks` t4 on t1.channel=t4.channel
...
ELSE 'others'
END AS playback_type, 
'no' AS ad_insertable_network, t1.program_title, t1.user_id, t1.start_time,SUM(duration)/3600 as Hours
from `fubotv-prod.data_insights.video_in_progress_via_va_view` t1
inner join users t2 on t1.user_id=t2.user_id and date(t1.start_time)=t2.day
inner join `fubotv-prod.data_insights.tmsid_genre_mapping` t3 on t1.tms_id=t3.tms_id
inner join `fubotv-dev.business_analytics.AdInsertable_Networks` t4 on t1.channel=t4.channel
...
SUM(Hours) AS Hours,
FROM results 
WHERE Date_range IN ( '2021-07-01', '2021-07-02', '2021-07-03', '2021-07-04', '2022-07-01', '2022-07-02', '2022-07-03', '2022-07-04') --- update the date range
GROUP BY 1,2,3,4,5,6,7,8
)

SELECT DISTINCT
CASE WHEN STRING(Date_range) LIKE '2021%' THEN "July 2021 YTD"
ELSE "July 2022 YTD" 
END AS Month,
network_name,
channel_name,
