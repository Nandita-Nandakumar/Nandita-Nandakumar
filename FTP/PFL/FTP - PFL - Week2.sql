...
AND (v.start_time between '2022-02-25' and '2022-02-27'
OR TIMESTAMP_ADD(v.start_time, INTERVAL v.duration SECOND) between '2022-02-25' and '2022-02-27')
AND tms_id = 'EP041888580002'
GROUP BY 1,2
)
...
WHERE receivedat >= '2022-02-25' and receivedat <= '2022-02-26'
...
AND properties_data_event_context_program_id = 'EP041888580002'
...
WHERE DATE(start_time,'America/New_York') IN ('2022-02-25')
...
WHERE v.playback_type not like '%dvr%' AND v.playback_type not like '%vod%' AND v.playback_type not like '%lookback%' -- include live & startover
AND lower(v.device_category) IN ('roku','web','iphone','android_tv', 'android_phone','android_tablet', 'ipad', 'fire_tv', 'smart_tv')
AND (v.start_time between '2022-02-25' and '2022-02-28'
OR TIMESTAMP_ADD(v.start_time, INTERVAL v.duration SECOND) between '2022-02-25' and '2022-02-28')
AND tms_id = 'EP041888580002'
GROUP BY 1
)
...
WHERE receivedat >= '2022-02-25' and receivedat <= '2022-02-27'
AND lower(properties_device_category) IN ('roku','web','iphone','android_tv','android_phone','android_tablet', 'ipad', 'fire_tv', 'smart_tv')
AND event IN ('ui_interaction')
AND properties_event_sub_category IN ('ftp_game')
AND properties_data_event_context_component = 'ftp_game_question'  
